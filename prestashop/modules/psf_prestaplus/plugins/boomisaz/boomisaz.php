...
class Prestaplus_boomisaz extends PrestaplusPlugin
{
    ...
    public function init()
    {
        ...
        // Faraket Api
        $this->faraketProductUnique = 'ps__prestafa__boomisaz';
        $this->hooks = array(
            'actionDispatcher',
    ...
    public function hookDisplayBackOfficeHeader()
    {
		...
		if ($this->isOlderThanPS1770()) {
			if (method_exists($this->context->controller, 'addJquery')) {
				$this->context->controller->addJS(Media::getJqueryPath(null, null, true), false);
			}
        ...
        if ($this->context->language->iso_code == 'fa')
        {
            ...
            if( $tinymce_status ){
                ...
                Media::addJsDef(array('psfplus_editor_skin_tinymce' => $skin_tinymce));

                if( version_compare(_PS_VERSION_, '1.7.3', '<') === true) {
                  $tinyJs = '<script type="text/javascript" src="'. $this->_path. 'views/js/admin/tinymce-psf-plus-old.js" defer="defer"></script>';
                } else {
                  $tinyJs = '<script type="text/javascript" src="'. $this->_path. 'views/js/admin/tinymce-psf-plus-new.js" defer="defer"></script>';
                }
    ...
    {
        if ( $this->isNewTheme() ) {
			$this->context->controller->addJS($this->_path.'views/js/admin/moment.min.js', 'all');
			$this->context->controller->addJS($this->_path.'views/js/admin/jMoment.js', 'all');
			$this->context->controller->addJS($this->_path.'views/js/admin/persian-date.min.js', 'all');
			
			if ($this->isOlderThanPS1770()) {
				$this->context->controller->addJS($this->_path.'views/js/admin/persian-datepicker.min.js', 'all');
				$this->context->controller->addCSS($this->_path.'views/css/admin/persian-datepicker.min.css', 'all');  
			}
        }
        if( $this->context->language->is_rtl )
        {
            if (version_compare(_PS_VERSION_, '1.7.3', '<') === true) {
              ...
              $this->context->controller->addCss($this->_path.'/views/css/admin/prestaplus-fix-rtl.css');

              if ( version_compare(_PS_VERSION_, '1.7.5', '>') === true ) {
                  $this->context->controller->addCss($this->_path.'/views/css/admin/version/1750.css');
              } else if( version_compare(_PS_VERSION_, '1.7.4', '>') === true ) {
                  $this->context->controller->addCss($this->_path.'/views/css/admin/version/1740.css');
              }
            }
            $this->context->controller->addCss($this->_path.'/views/css/admin/prestaplus-font.css');
        }
    ...
    public function hookDisplayHeader()
    {
        if ( $this->context->language->is_rtl)
        {
            ...
            if ( version_compare(_PS_VERSION_, '1.7.0', '>=') == false ) {
            ...
            } elseif ($this->context->shop->theme_name == 'classic') {
                $this->context->controller->addCss($this->_path.'views/css/prestaplus-fix.css');
                ...
                if (version_compare(_PS_VERSION_, '1.7.3', '>=') == false) {
                    $this->context->controller->registerStylesheet(
    ...
    public function getDate( $date )
    {
        ...
        {
            if( $dateArray['year'] > 1000 && $dateArray['year'] < 1600 )
            {
	...
	public function isOlderThanPS1770()
	{
		return Tools::version_compare(_PS_VERSION_, '1.7.7.0', '<') == true;
	}
}
