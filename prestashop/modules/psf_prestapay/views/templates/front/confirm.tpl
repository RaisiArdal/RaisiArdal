...
{block name='page_title'}
    <header class="page-header">
        <h1>{l s='اطلاعات سفارش' mod="psf_prestapay"}</h1>
    </header>
{/block}
        ...
        <div class="psf-payment-content">
            {if isset($id_order) }
                <div id="psf-content-top">
                    <h3>{l s='سفارش شما با موفقیت ثبت شد.' mod="psf_prestapay"}</h3>
                </div>
            ...
            {else}
                <div id="psf-content-top">
                    ...
                    <p>
                        <strong>{l s='شماره پیگیری: ' mod="psf_prestapay"} </strong>{$reference_payment}<br>
                        <strong>{l s='شماره تراکنش: ' mod="psf_prestapay"} </strong>{$authority_payment}
                    </p>
                </div>
                <div id="psf-content-message">
                    {if isset($messagePlugins)}
                        <p>{$messagePlugins nofilter}</p>
                    {/if}
                </div>
                <div id="psf-content-bottom">
                    <p>
